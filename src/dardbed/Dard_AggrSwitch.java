...
public class Dard_AggrSwitch extends Node {
	private int k;
	...
	public Dard_AggrSwitch(Address address, int k) {
		this.addr = address;
		this.k = k;
		...
		this.buffer_sz = k * 100 * Link.PSIZE;
	...
	private boolean receive(NetworkEvent event) {
		...
		if (event.getPDU().type == PDU.TCP) {
			...
			} else if (SourceRouting.getPath(fid) != null) {
				Path p = SourceRouting.getPath(fid);
				link = upports.get(p.core);
			} else {
