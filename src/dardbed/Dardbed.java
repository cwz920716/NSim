...
public class Dardbed {
	public static void main(String[] args) {
		int k = 16;
		Dard.INIT(k); // GFF.INIT(k); // Hedera.k = k;
		Dard_Topology topo = new Dard_Topology(k);
		int xmax = 1;
		...
		for (int i = 1; i <= k; i++)
			for (int j = 1; j <= k / 2; j++)
				for (int m = 1; m <= k / 2; m++) {
					...
					Dard_Endhost end1 = topo.getHost(addr);
					// System.out.println(end1.getName());
					for (int x = 0; x < xmax; x++) {
						Dard_Endhost end2 = topo.getHost(Trace.Randbij(addr));
