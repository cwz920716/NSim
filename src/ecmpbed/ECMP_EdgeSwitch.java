...
public class ECMP_EdgeSwitch extends Node {
	private int k;
	...
	public ECMP_EdgeSwitch(Address address, int k) {
		this.addr = address;
		this.k = k;
	...
	private boolean send(NetworkEvent event) {
		if (event.getRelatedLink().failed()) {
	...
	private boolean receive(NetworkEvent event) {
		...
		if (event.getPDU().type == PDU.TCP) {
			...
			if (destpod == addr.getPod() && destedge == addr.getEdge()) {
				...
				if (Fails.F != null && !Fails.F.okpath(addr, m.src, m.dest, desthost + k / 2)) {
					// System.out.println(this.getName() + "has to drop this packet due to network failure");
					return true;
				}
			} else {
				...
				if (Fails.F != null && !Fails.F.okpath(addr, m.src, m.dest, m.hash(k / 2, this.getAddress()))) {
					...
					if (num == 0) {
						// System.out.println(this.getName() + "Xhas to drop this packet due to network failure");
						return true;
					} else {
						int substitude = m.hash(num, this.getAddress()), next = 0;
						for (int i = 1; i <= k / 2; i++) {
